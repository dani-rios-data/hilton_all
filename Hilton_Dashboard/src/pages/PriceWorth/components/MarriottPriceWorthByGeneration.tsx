...
interface MarriottPriceWorthByGenerationProps {
  data: PriceWorthData[];
}

const GENERATIONS = ['Millennials', 'Gen X', 'Boomers'];

const MarriottPriceWorthByGeneration: React.FC<MarriottPriceWorthByGenerationProps> = ({ data }) => {
      ...
      GENERATIONS.forEach(generation => {
        ...
        if (generationData) {
          result[`${generation}`] = processValue(
            selectedMetric === 'price' ? generationData.marriottPrice : generationData.marriottWorth
          );
        }
      });
    ...
    return (
      <div className="p-4 bg-white rounded shadow-sm">
        <h3 className="mb-3 text-lg" style={{ fontFamily: 'Georgia, serif', color: colors.hiltonBlue }}>
          Marriott {selectedMetric === 'price' ? 'Price' : 'Worth'} Over Time by Generation
        </h3>
          ...
          >
            ...
            {GENERATIONS.map((generation, index) => (
              <Line 
                ...
                type="monotone" 
                dataKey={generation} 
                stroke={[colors.turquoise, colors.teal, colors.blueTint][index % 3]} 
                {...lineConfig}
              />
            ))}
          </LineChart>
...
export default MarriottPriceWorthByGeneration; 