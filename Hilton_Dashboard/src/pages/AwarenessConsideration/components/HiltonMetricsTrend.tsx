...
interface HiltonMetricsTrendProps {
  awarenessData: AwarenessData[];
  considerationData: ConsiderationData[];
}

const HiltonMetricsTrend: React.FC<HiltonMetricsTrendProps> = ({ awarenessData, considerationData }) => {
  const trendData = useMemo(() => {
    if (!awarenessData || !considerationData || awarenessData.length === 0 || considerationData.length === 0) {
      return [];
    }

    // Filtrar datos de Hilton y Unaided Awareness
    const awarenessFiltered = awarenessData.filter(
      item => item.brand === 'Hilton' && item.category === 'Unaided Awareness'
    );

    // Filtrar datos de Hilton y Consideration
    const considerationFiltered = considerationData.filter(
      item => item.brand === 'Hilton' && item.category === 'Consideration'
    );
    ...
    return (
      <div className="p-4 bg-white rounded shadow-sm">
        <h3 className="mb-3 text-lg" style={{ fontFamily: 'Georgia, serif', color: colors.hiltonBlue }}>
          Hilton: Awareness vs Consideration Over Time
        </h3>
...
export default HiltonMetricsTrend; 